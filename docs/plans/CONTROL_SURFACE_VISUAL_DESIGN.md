...
| Arp Slice | A left-to-right step strip where each slice appears as four equal-height strikes before the next slice changes height | Highlight advances chronologically through every repeated strike from left to right | Four grouped repetitions per visible slice with one active strike | Sequence length / repeat bars |
...
| Buffer Delay Dub Burst | Dub sound-system cabinet whose upper tape reels feed a large woofer and three blooming orbital echoes | Tape reels turn, the woofer pushes outward, and delayed red/gold/green wavefront lamps bloom into space; feedback controls their persistence | Cabinet, twin reels, woofer, and three nested echo fronts | Delay division / feedback where available |
