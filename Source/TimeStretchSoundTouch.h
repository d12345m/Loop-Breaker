...
public:
    ...
    // Number of input frames still retained by SoundTouch's processing stages.
    int numUnprocessedSamples() const
    {
        return (int) st.numUnprocessedSamples();
    }

    // Expected output frames generated by one input frame at the current
    // tempo/rate/pitch settings.
